...
public class MyGLRenderer implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        Matrix.setLookAtM(mViewMatrix, 0, 0, 0, 5f, 0, 0, -5, 0, 1, 0);
...
//        }
    ...
    private float[] getVertexData(int row, int col) {
        ...
        //  double[][] spherePoints = new double[][] {{inc, phi}, {inc+ theta, phi}, {inc + theta, phi + inc / 2}, {inc, phi + theta / 2}};


        // double[][] spherePoints = new double[][] {{inc, phi}, {theta + inc, phi}, {theta + inc, phi + inc/2}, {theta, phi + inc}};
        ...
        return new float[] {
               ...
               /* (float) (vertexPoints[0][0]),(float) (vertexPoints[0][1]), (float) (vertexPoints[0][2]),
                ...
                (float) (vertexPoints[0][0]),(float) (vertexPoints[0][1]), (float) (vertexPoints[0][2]),
                getColor(), getColor(), getColor(), 1.0f,
...
//        return new double[] {Math.cos(p[0]) * Math.sin(p[1]), Math.sin(p[0]) * Math.sin(p[1]), Math.cos(p[1])};
        return new double[] {Math.cos(p[0]), Math.sin(p[0]), Math.cos(p[1])};
